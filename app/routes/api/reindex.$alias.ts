...
export const action = async ({ params, context }: ActionFunctionArgs) => {
  ...
  if (!canModify(user, connectionConfig.ownerScope)) {
    return new Response("Not authorized to reindex this connection", {
      status: 403,
    });
  }
  ...
  try {
    ...
    console.error(`[reindex] Failed for ${provider}/${bucketName}:`, error);
