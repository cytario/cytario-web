...
/**
 * Deployment-declared platform-managed buckets: the only buckets whose
 * connections may be created with `managedExternally`. Unset disables the
 * managed path entirely.
 */
...
/** Sentinel `createdBy` for service-created connections: the column is a
 * plain string (no FK), so the sentinel is safe to display as creator name. */
const SERVICE_CREATED_BY = "admin-portal";

/** Safe projection of a connection row for a service caller: identifiers and
 * org-tenant fields only — no role ARNs, endpoints, or catalog metadata. */
 ...
 * The caller is a trusted service presenting the deployment's shared
 * secret, so there is no session-user visibility filter (the admin portal
 * legitimately sees every connection of the orgs it manages).
 */
...
/**
 * Service-to-service create: `POST /api/connections`, called by the admin
 * portal during onboarding. The organization comes from the payload — the
 * caller is a trusted service, not a session.
 *
 * Idempotent: the portal retries creates after network failures, so an
 * existing (organization, providerConnectionId, bucketName, prefix) tuple
 * returns `200 { created: false }`, not a conflict.
 *
 * `managedExternally: true` skips the bucket-policy apply entirely — the
 * bucket's policy is managed by Terraform in the portal, and a web-app
 * apply would race or clobber it.
 */
export async function action(args: ActionFunctionArgs): Promise<Response> {
  ...
  // The org must resolve against Keycloak before anything is written: the
  // shared-secret channel is authenticated, not identified — a leaked secret
  // must not be able to mint rows for phantom tenants.
  ...
  // A managedExternally create for a bucket outside the allowlist is rejected
  // outright rather than silently skipping validation — the skip exists
  // because the portal already resolved the references.
  ...
  const result = await createConnectionWithCatalogValidation(
    ...
    // create the portal itself already resolved the references.
    "",
    {
