...
describe("EmptyState (migrated from Placeholder)", () => {
  ...
  test("renders icon if provided", () => {
    render(<EmptyState title="Test" description="Desc" icon="X" />);
    // EmptyState renders the icon
    const svg = document.querySelector("svg");
    expect(svg).toBeInTheDocument();
  });

  test("renders action if provided", () => {
    render(<EmptyState title="Test" description="Desc" action={<button>Click me</button>} />);
