  ...
  useEffect(() => {
    ...
    const onKeyUp = (e: KeyboardEvent) => {
      if (e.code === "Space") {
        ...
        // Only restore if the mode is still "view" — if the user changed modes
        // while Space was held, the savedMode is stale and must not be restored.
    ...
    const onBlur = () => {
      // If Space was held when the window lost focus, restore the saved mode so the
      // user doesn't get stuck in "view".
