...
describe("Histogram", () => {
  ...
  test("expands range to a committed contrast max above the auto-fit domain", () => {
    // Auto-fit domain is 255, but the user committed Max=50000 (clamped to the
    // 16-bit dtype ceiling, not the domain — see C-232). The slider/tick range
    // must grow to include it so the handle stays on-track.
    (useViewerStore as Mock).mockImplementation((selector) => {
      if (selector === select.channelsState) {
        return {
          Red: {
            ...mockChannelsState.Red,
            domain: [0, 255] as [number, number],
            contrastLimits: [0, 50000] as [number, number],
          },
        };
      }
      if (selector === select.selectedChannel) {
    ...
    const slider = screen.getByTestId("domain-slider");
    const domain = JSON.parse(slider.getAttribute("data-domain") || "[]");
    expect(domain).toEqual([0, 50000]);
  });

  test("caps the expanded range at the dtype ceiling", () => {
    // A contrast max beyond the 16-bit ceiling (65535) cannot drive the range
    // past the dtype value space.
    (useViewerStore as Mock).mockImplementation((selector) => {
      if (selector === select.channelsState) {
        return {
          Red: {
            ...mockChannelsState.Red,
            domain: [0, 255] as [number, number],
            contrastLimits: [0, 999999] as [number, number],
          },
        };
      }
      if (selector === select.selectedChannel) {
  ...
  test("keeps the auto-fit domain when no contrast max exceeds it", () => {
    // Low-signal fluorescence: contrast limits sit within the auto-fit domain,
    // so the range stays at the domain and the histogram is not squashed.
    (useViewerStore as Mock).mockImplementation((selector) => {
      if (selector === select.channelsState) {
        return {
          Red: {
            ...mockChannelsState.Red,
            domain: [0, 4000] as [number, number],
            contrastLimits: [100, 2000] as [number, number],
          },
        };
      }
      if (selector === select.selectedChannel) {
    ...
    const slider = screen.getByTestId("domain-slider");
    const domain = JSON.parse(slider.getAttribute("data-domain") || "[]");
    expect(domain).toEqual([0, 4000]);
  });

  test("ignores contrast limits of hidden channels when expanding the range", () => {
    // A hidden channel with a huge contrast max must not stretch the range for
    // the visible signal.
    (useViewerStore as Mock).mockImplementation((selector) => {
      if (selector === select.channelsState) {
        return {
          Red: {
            ...mockChannelsState.Red,
            domain: [0, 255] as [number, number],
            contrastLimits: [0, 200] as [number, number],
            isVisible: true,
          },
          Green: {
            ...mockChannelsState.Green,
            domain: [0, 255] as [number, number],
            contrastLimits: [0, 60000] as [number, number],
            isVisible: false,
          },
        };
      }
