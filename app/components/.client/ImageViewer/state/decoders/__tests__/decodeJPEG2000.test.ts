...
describe("decodeJPEG2000", () => {
  it("decodes a JPEG2000 image and returns the correct pixel data and frame info", async () => {
    ...
    const expectedPixelCount = result.info.width * result.info.height * result.info.componentCount;
    ...
    expect(result.pixels.length).toBeGreaterThan(compressedImageFrame.length);
  });
});
