...
const useViewerRegistryStore = create<ViewerRegistryStore>()(
  devtools(
    (set, get) => ({
      viewers: {},
      registerViewer: ({ id, url, offsetsUrl, credentials, connectionConfig }) => {
...
export const ViewerStoreProvider = ({
  ...
  const store = useMemo(
    () => registerViewer({ id: resourceId, url, offsetsUrl, credentials, connectionConfig }),
    [resourceId, url, offsetsUrl, credentials, connectionConfig, registerViewer],
  );
