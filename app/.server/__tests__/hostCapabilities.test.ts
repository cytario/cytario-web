...
describe("HostCapabilities (SDS-CY-010097/010098/010099)", () => {
  ...
  test("assumeComputeRole projects a memory ladder from the max-resources blob", async () => {
    getProviderCatalogMock.mockResolvedValue({
      providerConnections: [],
      providerRoles: [],
      computeProviders: [
    ...
    expect(session.maxResources?.memorySteps).toEqual(["16Gi", "32Gi", "64Gi", "192Gi"]);
  });

  test("assumeComputeRole omits the ladder on a payload that predates it", async () => {
    getProviderCatalogMock.mockResolvedValue({
      providerConnections: [],
      providerRoles: [],
      computeProviders: [
    ...
    expect(session.maxResources?.memorySteps).toBeUndefined();
  });
