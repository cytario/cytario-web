...
describe("isZarrPath", () => {
  describe("detection with .zarr extension", () => {
    test("detects URL ending with .zarr", () => {
      expect(isZarrPath("https://bucket.s3.amazonaws.com/data.zarr")).toBe(true);
    });

    test("detects URL ending with .zarr/", () => {
      expect(isZarrPath("https://bucket.s3.amazonaws.com/image.zarr/")).toBe(true);
    });

    test("detects URL with .zarr in path", () => {
      expect(isZarrPath("https://bucket.s3.amazonaws.com/images/sample.zarr/0/0/0")).toBe(true);
    });
    ...
    test("detects AWS S3 virtual-hosted URL with .zarr", () => {
      expect(isZarrPath("https://my-bucket.s3.us-west-2.amazonaws.com/data.zarr")).toBe(true);
    });
  ...
  describe("non-zarr URLs and paths", () => {
    test("returns false for OME-TIFF files", () => {
      expect(isZarrPath("https://bucket.s3.amazonaws.com/image.ome.tif")).toBe(false);
    });

    test("returns false for regular TIFF files", () => {
      expect(isZarrPath("https://bucket.s3.amazonaws.com/image.tiff")).toBe(false);
    });

    test("returns false for parquet files", () => {
      expect(isZarrPath("https://bucket.s3.amazonaws.com/data.parquet")).toBe(false);
    });
    ...
    test("returns false for paths with 'zarr' as substring (no dot)", () => {
      expect(isZarrPath("https://bucket.s3.amazonaws.com/zarring/data")).toBe(false);
    });
    ...
    test("does not match .zarr as part of a longer extension", () => {
      expect(isZarrPath("https://example.com/.zarr-config/settings")).toBe(false);
    });
  });
  ...
  describe("edge cases", () => {
    ...
    test("handles URLs with query parameters", () => {
      expect(isZarrPath("https://bucket.s3.amazonaws.com/data.zarr?version=1")).toBe(true);
    });
